...
public class ISO8583 {
    ...
    public byte[] setField( int field, String value) {
        ...
        {
            Log.d(TAG, "no attribute_array set!");
            return null;
        }
        if( value.length() == 0 )
        {
            Log.d(TAG, "invalid value(len:0) of field:" + field);
            return null;
        }
        byte[] bytes = null;
        int len = 0;
        ...
        if( 0 != (attribute_array[field][ATTR_INDEX_TYPE] & TYPE_BCD) ) {
            ...
            test = Utility.byte2HexStr(bytes);
            ...
            Log.d( TAG, "type " + Integer.toHexString(attribute_array[field][ATTR_INDEX_TYPE]) + " invalided" );
    ...
    public byte[] getFieldCust( int field, String value) {
    ...
    public int fieldType(int field)
    {
        ...
        if (0 != (attribute_array[field][ATTR_INDEX_TYPE] & TYPE_ASC))
            return TYPE_ASC;
        else if (0 != (attribute_array[field][ATTR_INDEX_TYPE] & TYPE_BCD))
            return TYPE_BCD;
...
//        String valueTrimed = new String(value);
        ...
        if( retBytes == null ) {
            Log.d( TAG, "type " + Integer.toHexString(attribute_array[field][ATTR_INDEX_TYPE]) + ", len " + value.length + " of field: " + field+ " invalid :" + wantLen );
            validField[field] = false;
        } else {
    ...
    public boolean unpack( byte[] packet, int offset ){
        ...
        isoData = new HashMap<Integer, String>();
        ...
        for( int i = 1; i <= ISO_BIT_MAX; i++ ){
            ...
            {
                unpackValidField[i] = true;
                Log.d(TAG, "Mark Field:" + i);
                ...
                } else if( 0 != (attribute_array[field][ATTR_INDEX_TYPE] & TYPE_BIN) )
                {
                    ...
                    String test = Utility.byte2HexStr(packet, fieldOffset, length);
    ...
    public byte[] appendF( int field, int tag, byte[] value ) {
        ...
        if (len <= 0x7F) {
            ...
            Log.d(TAG, "invalid length:" + len + " of TAG:" + Integer.toHexString(tag) + " to Field:" + field);
            return null;
        }
        lenTag = sTag.length();
        lenLen = offset;
    ...
    public byte[] makePacket( Map<Integer, String> data, PACKET_TYPE type) {
        int fieldIndex;
        String fieldValue;
        byte[] tmp;
        String x ;

        for( Map.Entry<Integer, String> field : data.entrySet() )
        {
            fieldIndex = field.getKey();
            fieldValue = field.getValue();
            tmp = setField( fieldIndex, fieldValue );

            if( tmp == null )
            {
                Log.d(TAG, "error of index:" + fieldIndex + ", value:" +  fieldValue );
            }
        }

        byte[] output = getPacket(header, tail, type );
        // System.arraycopy(packet,0,output,0, output.length);
        return output;
    }
